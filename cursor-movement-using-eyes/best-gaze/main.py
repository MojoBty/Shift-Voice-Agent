...
try:
    from Quartz import (CGEventCreateMouseEvent, CGEventPost,
                        ...
                        CGWindowListCreateImage, CGRectMake, CGMainDisplayID,
...
CAPTURED_TEXT_PATH = os.path.join(DIR, "captured_text.txt")
...
# Per-axis blend: iris is reliable horizontally, nose/head is reliable vertically
HEAD_W_X = 0.3
HEAD_W_Y = 0.35
DEAD_ZONE = 1.5
...
# ── Text capture on lock ──────────────────────────────────────────
REGION_HW = 350   # half-width of screenshot region
REGION_HH = 150   # half-height of screenshot region
TEXT_WIN = "Captured Text"
TEXT_WIN_W = 600
TEXT_WIN_H = 340


def _ocr(cg_image):
    """Run macOS Vision OCR on a CGImage, return list of text lines."""
    results_out = []
    ...
    def _handler(request, error):
        ...
        for obs in observations:
            ...
            if top:
                results_out.append(top[0].string())
...
def capture_text_at(x, y):
    """Screenshot the area around (x, y), OCR it, save to file, return text."""
    if not QUARTZ_OK or not VISION_OK:
        msg = "(text capture unavailable -- missing Quartz or Vision)"
        print(f"  {msg}")
        with open(CAPTURED_TEXT_PATH, "w") as f:
            f.write(msg)
        return msg

    disp = CGMainDisplayID()
    sw = CGDisplayPixelsWide(disp)
    sh = CGDisplayPixelsHigh(disp)

    left = max(0, int(x - REGION_HW))
    top = max(0, int(y - REGION_HH))
    right = min(sw, int(x + REGION_HW))
    bottom = min(sh, int(y + REGION_HH))
    w, h = right - left, bottom - top
    if w < 10 or h < 10:
        return ""

    cg_image = CGWindowListCreateImage(
        CGRectMake(left, top, w, h),
    ...
    with open(CAPTURED_TEXT_PATH, "w") as f:
        f.write(text)
    print(f"  Captured {len(lines)} lines -> {CAPTURED_TEXT_PATH}")
    return text


def show_text_window(text):
    """Display captured text in a floating OpenCV window."""
    frame = np.zeros((TEXT_WIN_H, TEXT_WIN_W, 3), dtype=np.uint8)
    frame[:] = (30, 30, 30)

    cv2.putText(frame, "LOCKED - Captured Text", (10, 22),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (100, 100, 255), 1)
    cv2.line(frame, (0, 30), (TEXT_WIN_W, 30), (80, 80, 80), 1)
    ...
    if not text:
        ...
        for i, line in enumerate(wrapped[:15]):
            y = 50 + i * 19
            cv2.putText(frame, line, (10, y),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.38, (220, 220, 220), 1)

    cv2.imshow(TEXT_WIN, frame)


def hide_text_window():
    """Close the captured-text window."""
    try:
        cv2.destroyWindow(TEXT_WIN)
    except cv2.error:
        pass
    ...
    while running:
        try:
            ...
            if not gaze_locked and ("hey agent" in txt or "hey, agent" in txt):
                ...
                text = capture_text_at(lock_x, lock_y)
...
def make_panel(status, locked):
    ...
    cv2.putText(p, "R=recal  L=lock/describe  Q=quit", (180, 22),
                cv2.FONT_HERSHEY_SIMPLEX, 0.3, (80, 80, 100), 1)
    return p


def run_tracking_loop(cal, shared_state):
    """
    Run the eye tracking loop without wake-word or calibration.
    shared_state: dict with gaze_locked, lock_x, lock_y, running (all read/write as needed).
    Caller must set running=False to exit.
    """
    global blink_ctr, HEAD_W_X, HEAD_W_Y, filt_x, filt_y
    ...
    try:
        while shared_state.get("running", True):
            ...
            status = "NO FACE"
            gaze_locked = shared_state.get("gaze_locked", False)
            lock_x = shared_state.get("lock_x", screen_w // 2)
            lock_y = shared_state.get("lock_y", screen_h // 2)
            ...
            if key == ord("q"):
                ...
                if gaze_locked:
                    ...
                    shared_state["lock_x"] = lx
                    shared_state["lock_y"] = ly
                    print(f"\n  LOCKED at ({lx}, {ly})")
                    captured = capture_text_at(lx, ly)
                    show_text_window(captured)
                    on_describe = shared_state.get("on_lock_describe")
                    if on_describe and captured:
                        threading.Thread(target=on_describe, args=(captured,), daemon=True).start()
